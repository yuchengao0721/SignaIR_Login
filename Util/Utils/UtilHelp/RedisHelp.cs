...
namespace UtilHelp
{
    ...
    public class RedisHelper
    {
        ...
        private static readonly object lock_redis = new object();
        /// <summary>
        /// 获取 Redis 连接对象
        /// </summary>
        /// <returns></returns>
        ...
        static RedisHelper()
        {
            var config = new ConfigurationOptions
            {
                ...
                ConnectTimeout = 150,
                SyncTimeout = 150,
                Password = AppsetingHelp.GetSectionValue("RedisPassWord"),//Redis数据库密码
                EndPoints = { AppsetingHelp.GetSectionValue("RedisHost") }
            };
            _connMultiplexer = ConnectionMultiplexer.Connect(config);
            DefaultKey = AppsetingHelp.GetSectionValue("Redis.DefaultKey");
            AddRegisterEvent();
        }

        private RedisHelper(int db = -1)
        {
        ...
        {
            redisKey = AddKeyPrefix(redisKey);
            var json = Json.ToJson(redisValue);
        ...
        {
            redisKey = AddKeyPrefix(redisKey);
            return Json.ToObject<T>(_db.StringGet(redisKey));
        }
        ...
        {
            redisKey = AddKeyPrefix(redisKey);
            return Json.ToObject<T>(await _db.StringGetAsync(redisKey));
        }
        ...
        {
            redisKey = AddKeyPrefix(redisKey);
            return Json.ToObject<T>(_db.HashGet(redisKey, hashField));
        }
        ...
        {
            redisKey = AddKeyPrefix(redisKey);
            return Json.ToObject<T>(await _db.HashGetAsync(redisKey, hashField));
        }
        ...
        {
            redisKey = AddKeyPrefix(redisKey);
            return Json.ToObject<T>(_db.ListLeftPop(redisKey));
        }
        ...
        {
            redisKey = AddKeyPrefix(redisKey);
            return Json.ToObject<T>(_db.ListRightPop(redisKey));
        }
        ...
        {
            redisKey = AddKeyPrefix(redisKey);
            return _db.ListRightPush(redisKey, Json.ToJson(redisValue));
        }
        ...
        {
            redisKey = AddKeyPrefix(redisKey);
            return _db.ListLeftPush(redisKey, Json.ToJson(redisValue));
        }
        ...
        {
            redisKey = AddKeyPrefix(redisKey);
            return Json.ToObject<T>(await _db.ListLeftPopAsync(redisKey));
        }
        ...
        {
            redisKey = AddKeyPrefix(redisKey);
            return Json.ToObject<T>(await _db.ListRightPopAsync(redisKey));
        }
        ...
        {
            redisKey = AddKeyPrefix(redisKey);
            return await _db.ListRightPushAsync(redisKey, Json.ToJson(redisValue));
        }
        ...
        {
            redisKey = AddKeyPrefix(redisKey);
            return await _db.ListLeftPushAsync(redisKey, Json.ToJson(redisValue));
        }
        ...
        {
            var sub = _connMultiplexer.GetSubscriber();
            return sub.Publish(channel, Json.ToJson(message));
        }
        ...
        {
            var sub = _connMultiplexer.GetSubscriber();
            return await sub.PublishAsync(channel, Json.ToJson(message));
        }
